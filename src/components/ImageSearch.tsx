    ...
    <div className={isGoogle ? 'w-full' : 'h-full flex flex-col bg-white dark:bg-gray-800 rounded-lg shadow-lg'}>
      {isGoogle ? (
        <div className="flex flex-col items-center w-full min-w-0 px-1">
          <div className="text-2xl sm:text-4xl md:text-5xl font-semibold tracking-tight text-gray-900 mb-4 sm:mb-8 select-none text-center truncate max-w-full">
            OSINT WebSearch
          </div>

          <div className="w-full max-w-2xl min-w-0">
            {uploadPreview && (
              <div className="mb-3 flex items-center gap-2 p-2 bg-gray-50 rounded-lg min-w-0">
                <img 
                  src={uploadPreview} 
                  alt="Preview" 
            ...
            <div className="relative bg-white rounded-full shadow-lg border border-gray-300 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-200 min-w-0">
              <textarea
                ref={textareaRef}
                value={searchQuery}
                onChange={handleTextareaChange}
                onKeyDown={handleKeyDown}
                placeholder={uploadPreview ? 'Search with image...' : 'Name, username or keyword...'}
                disabled={loading || uploading}
                className="w-full min-w-0 bg-transparent text-gray-900 pl-4 pr-20 sm:pl-5 sm:pr-24 py-3 resize-none outline-none placeholder-gray-400 disabled:opacity-50 rounded-full text-base sm:text-inherit"
                rows={1}
                style={{ maxHeight: '200px', minHeight: '44px' }}
              />
              
              <div className="absolute right-1.5 bottom-1.5 sm:right-2 sm:bottom-2 flex items-center gap-1.5 flex-shrink-0">
                {/* Image Upload Button */}
                <input
                  ref={fileInputRef}
                  type="file"
